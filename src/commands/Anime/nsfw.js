...
module.exports = {
    name: "nsfw",
    category: "Anime",
    aliases: [ "nsfws","hentai","hentais" ],
    description: "Sends random nsfws anime pics",
    args: false,
    usage: "",
    permission: [],
    owner: false,
    ...
    execute: async (message, args, client, prefix) => {
      try{
        try {
            ...
            console.log(`${user} used NSFW Command TIME: ${date.toLocaleString()}`);
        } catch (err) {
            console.log(err);
        }
        const anime = randomanime.nsfw();
        const attachment = new MessageAttachment(anime, 'SPOILER_FILE.jpg');
        return message.channel.send({ files: [attachment] }).catch(err => console.log(err));
