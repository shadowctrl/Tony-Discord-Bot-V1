...
module.exports = {
    ...
    run: async(client, message) => {
        ...
        if (!command) return;
        if (!message.guild.me.permissions.has(Permissions.FLAGS.SEND_MESSAGES)) return await message.author.dmChannel.send({
        ...
        if (!message.guild.me.permissions.has(Permissions.FLAGS.EMBED_LINKS)) return await message.channel.send({
        ...
        if (command.owner && message.author.id !== `${client.owner}`) {
            embed.setDescription("Only ! Murali Anand#5615 can use this command!");
            return message.channel.send({
                embeds: [embed]
            });
        }
        ...
        if (!message.channel.nsfw && command.nsfw) {
            embed.setDescription("Use This Command in NSFW Enabled Text Channel!");
            return message.channel.send({
                embeds: [embed]
            });
        }
